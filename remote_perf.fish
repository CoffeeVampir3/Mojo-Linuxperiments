...
ssh $REMOTE_USER@$REMOTE_HOST "cd $REMOTE_PATH && env MOJO_ENABLE_RUNTIME=0 pixi run mojo build -I . -D ASSERT=all $TARGET && echo '=== PERF STAT ===' && perf stat -e assists.fp,assists.sse_avx_mix,ld_blocks.store_forward,machine_clears.count,dtlb_load_misses.walk_completed,mem_inst_retired.split_loads,instructions,cycles ./$BINARY"
