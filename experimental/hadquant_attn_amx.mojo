"""Chunked AMX int8 GQA attention — async dispatch via PoolFence.
...
Worker processes ALL positions for its KV group range. Public API
dispatches once and returns PoolFence immediately — no internal join.
Output quantization (f32 → i8) is the caller's responsibility post-join.
...
def attn_scratch_bytes_amx[num_heads: Int, num_kv_heads: Int, head_dim: Int,
                           num_workers: Int = 1,
                           max_seq_len: Int = 1]() -> Int:
    """Scratch required for chunked AMX attention.
    row_f32 holds max_seq_len positions of f32 output.
    """
    ...
    return max_seq_len * q_cols * size_of[Float32]() + num_workers * per_worker


# ============================================================================
# Dispatched Kernel — processes ALL positions for its KV groups
# ============================================================================

def had_attn_groups_amx[num_heads: Int, num_kv_heads: Int, head_dim: Int, max_seq: Int](
    ctx_addr: Int, unused: Int, group_start: Int, group_end: Int,
    worker_scratch: Int, timing_addr: Int,
):
    """Process all positions for KV groups [group_start, group_end).
    Writes f32 output to row_f32[m * q_cols + h * head_dim + d].
    """
    ...
    # Scratch layout (reused per position)
    ...
    # Tile config (once per worker, reused across positions)
    ...
    for m in range(seq_len):
        ...
        for g in range(group_start, group_end):
            ...
            for chunk_start in range(0, context, CHUNK):
                ...
                # Fused: Exp + V scale + quantize to VNNI + accumulate l
            ...
            # =================================================================
            # 4. Final normalization — write to row_f32[m * q_cols + ...]
            # =================================================================
            ...
            for d in range(head_dim):
                ...
                for hi in range(gqa_factor):
                    var h = g * gqa_factor + hi
                    ctx[].row_f32[m * q_cols + h * head_dim + d] = row[hi]


# ============================================================================
# Public API — async dispatch, returns PoolFence
# ============================================================================
...
def int8_gqa_attention_amx[num_heads: Int, num_kv_heads: Int, head_dim: Int, max_seq: Int,
    ...
    """Async AMX int8 GQA attention. Dispatches and returns immediately.
    ...
    Scratch >= attn_scratch_bytes_amx[..., num_workers, max_seq_len]().
    """
    ...
    var WORKERS_OFF = q.seq_len * q_cols * size_of[Float32]()
    ...
    # Single dispatch: workers loop over all positions internally
