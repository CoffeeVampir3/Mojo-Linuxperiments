...
def load_and_run[
    ...
    var model_opt = SmolLM2TP[tp, P].load(Path(MODEL_PATH), numa, numa_topo, pools^)
    if not model_opt:
        return
    ...
    var logits = model.forward(Int(tp_ptr), seq_len, 0, profile=True)
    ...
    for step in range(1, MAX_NEW_TOKENS):
        ...
        logits = model.forward(Int(tp_ptr), 1, pos, profile=False)
